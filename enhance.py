...
add_arg('--zoom',               default=1, type=int,                help='.')
add_arg('--rendering-tile',     default=128, type=int,              help='.')
add_arg('--rendering-overlap',  default=32, type=int,               help='.')
add_arg('--rendering-histogram',default=False, action='store_true', help='.')
add_arg('--type',               default='photo', type=str,          help='.')
add_arg('--model',              default='default', type=str,        help='.')
add_arg('--train',              default=False, type=str,            help='.')
add_arg('--train-scales',       default=0, type=int,                help='.')
add_arg('--train-blur',         default=None, type=int,             help='.')
add_arg('--train-noise',        default=None, type=float,           help='.')
add_arg('--train-jpeg',         default=[], nargs='+', type=int,    help='.')
add_arg('--epochs',             default=10, type=int,               help='.')
add_arg('--epoch-size',         default=72, type=int,               help='.')
add_arg('--save-every',         default=10, type=int,               help='.')
add_arg('--batch-shape',        default=192, type=int,              help='.')
add_arg('--batch-size',         default=15, type=int,               help='.')
add_arg('--buffer-size',        default=1500, type=int,             help='.')
add_arg('--buffer-fraction',    default=5, type=int,                help='.')
add_arg('--learning-rate',      default=1E-4, type=float,           help='.')
add_arg('--learning-period',    default=75, type=int,               help='.')
add_arg('--learning-decay',     default=0.5, type=float,            help='..')
add_arg('--generator-upscale',  default=2, type=int,                help='.')
add_arg('--generator-downscale',default=0, type=int,                help='.')
add_arg('--generator-filters',  default=[64], nargs='+', type=int,  help='.')
add_arg('--generator-blocks',   default=4, type=int,                help='.')
add_arg('--generator-residual', default=2, type=int,                help='.')
add_arg('--perceptual-layer',   default='conv2_2', type=str,        help='')
add_arg('--perceptual-weight',  default=1e0, type=float,            help='.')
add_arg('--discriminator-size', default=32, type=int,               help='.')
add_arg('--smoothness-weight',  default=2e5, type=float,            help='.')
add_arg('--adversary-weight',   default=5e2, type=float,            help='.')
add_arg('--generator-start',    default=0, type=int,                help='.')
add_arg('--discriminator-start',default=1, type=int,                help='.')
add_arg('--adversarial-start',  default=2, type=int,                help='.')
add_arg('--device',             default='cpu', type=str,            help='.')
    ...
    def add_to_buffer(self, f):
        ...
        try:
            ...
            if any(s < args.batch_shape for s in orig.size):
                raise ValueError('Try different image, aparently the image is too small {}'.format(orig.size))
        except Exception as e:
            warn('Could not load `{}` as image.'.format(filename),
                 '  - Idk WHy thos just happened.')
            self.files.remove(f)
            return
...
class SubpixelReshuffleLayer(lasagne.layers.Layer):
    """ Added code based by ajbrock: https://github.com/ajbrock/Neural-Photo-Editor/
    """
    ...
    def load_generator(self, params):
        ...
        for k, l in self.list_generator_layers():
            ...
            for p, v in zip(l.get_params(), params[k]):
                assert v.shape == p.get_value().shape, "Mismatch in numbers for layer {}.".format(k)
...
class NeuralEnhancer(object):
    ...
    def __init__(self, loader):
        if args.train:
            ...
            if len(args.files) == 0: error("Add iamge to enhance on the command-line.")
        ...
        try:
            ...
            for epoch in range(args.epochs):
                ...
                if (epoch+1) % args.save_every == 0:
                    print('  - saving current generator layers to disk...Uhhhhhh THis takes too long')
